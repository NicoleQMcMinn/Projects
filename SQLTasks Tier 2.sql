...
SELECT facid, name, membercost, monthlymaintenance, (membercost/monthlymaintenance) AS cost_as_pct_maintenance
FROM Facilities
WHERE membercost > 0 
...
SELECT
	...
	m.firstname,
	m.surname,
    m.firstname || ' - ' || f.name AS member_and_facility,
...
engine = sqlite3.connect('country_club.db')
...
ON b.facid = f.facid
WHERE memid <> 0
GROUP BY f.name, b.memid;""", engine)
